...
def main(args):
    ...
    fname = glue_util.cache_filename(args)
    if fname.exists():
        print('load attacks from cache ...')
        adv_data = attacks.load(fname)
        assert len(adv_data) == len(dev_data), (len(adv_data), len(dev_data))
    else:
        print('generate attacks ...')
        adv_data = attacks.initialize(args, base_rn).attack_dataset(dev_data)
        attacks.save(adv_data, fname)

    print('Running adversarial evaluation.')
    results['adv_acc'] = base_rn.evaluate(adv_data, args.batch_size)

    with open(args.output_dir/f'{args.task_name}_black_{args.max_num_words}.json', 'w') as f:
        json.dump(results, f)
